...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D1117", fontFamily: "'Inter', sans-serif", color: "#E6EDF3" }}>
      ...
      {editTarget   && <EditModal row={editTarget} onSave={handleEdit} onCancel={() => setEditTarget(null)} />}
      {deleteTarget && <DeleteDialog company={deleteTarget.company} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />}
      {showAdd      && <AddApplicationModal onSave={handleAdd} onCancel={() => setShowAdd(false)} />}

      <Header
        search={search}
        onSearchChange={setSearch}
        onLogout={() => { localStorage.removeItem("jt_auth"); setAuthed(false); }}
      />

      {/* Add application button */}
      <div className="add-btn-row" style={{ width: "100%" }}>
        <button
      ...
      <div className="main-padding">
        ...
        <div className="statcard-grid">
          <StatCard icon={<FileText size={24} />}  label="Total Applications" value={stats.total}       color="#6366F1" bg="rgba(99,102,241,0.15)"  trend={12}  />
          <StatCard icon={<Users size={24} />}      label="Interviews"         value={stats.interviews}  color="#8B5CF6" bg="rgba(139,92,246,0.15)"  trend={25}  />
          <StatCard icon={<Briefcase size={24} />}  label="Offers"             value={stats.offers}      color="#10B981" bg="rgba(16,185,129,0.15)"  trend={100} />
          <StatCard icon={<CircleX size={24} />}    label="Rejections"         value={stats.rejections}  color="#EF4444" bg="rgba(239,68,68,0.15)"   trend={7}   />
        </div>
        ...
        {/* Table */}
        {loading ? (
          <div style={{ color: "#8B949E", textAlign: "center", padding: "48px 0", fontSize: 14 }}>
